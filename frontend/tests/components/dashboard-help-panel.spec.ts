...
test.describe("HelpPanel Component", () => {
  ...
  test("should display help panel header", async ({ page }) => {
    ...
    await expect(
      page.getByText("Learn how to use QuizCrafter effectively"),
    ).toBeVisible()
  })

  test("should display About QuizCrafter section", async ({ page }) => {
    await expect(page.getByText("About QuizCrafter")).toBeVisible()
    await expect(
      page.getByText(
        "QuizCrafter uses LLMs to generate multiple-choice questions from your Canvas course materials.",
      ),
    ).toBeVisible()
    await expect(
      page.getByText(
  ...
  test("should display How It Works section with numbered steps", async ({
    ...
    await expect(
      howItWorksSection.getByText("Export to Canvas as a quiz"),
    ).toBeVisible()
  })
  ...
  test("should be accessible with proper heading structure", async ({
    ...
    const aboutSection = page.getByText("About QuizCrafter")
